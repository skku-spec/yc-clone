...
import { getTopCompanies, type Company } from "@/lib/companies-data";

const TOP_COMPANIES = getTopCompanies();
...
export default function TopCompaniesPage() {
  ...
  const filteredCompanies = useMemo(() => {
    if (!searchQuery) return TOP_COMPANIES;
    ...
    <div className="min-h-screen px-4 pb-16 pt-12 md:px-8 md:pt-16">
      <div className="mx-auto max-w-[1200px]">
        ...
        <p className="mb-8 max-w-[640px] font-['Outfit',sans-serif] text-[17px] font-light leading-relaxed text-[#16140f]/80">
          The most successful YC companies by valuation, impact, and growth. These
          companies are shaping the future across industries.
        </p>
        ...
        {filteredCompanies.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredCompanies.map((company) => (
              <TopCompanyCard key={company.slug} company={company} />
            ))}
          </div>
        )}
      </div>
...
function TopCompanyCard({ company }: { company: Company }) {
  return (
    <Link
