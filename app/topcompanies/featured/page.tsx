...
import { getFeaturedCompanies, type Company } from "@/lib/companies-data";

const FEATURED_COMPANIES = getFeaturedCompanies();
...
export default function FeaturedCompaniesPage() {
  ...
  const filteredCompanies = useMemo(() => {
    if (!searchQuery) return FEATURED_COMPANIES;
    ...
    <div className="min-h-screen px-4 pb-16 pt-12 md:px-8 md:pt-16">
      <div className="mx-auto max-w-[1200px]">
        ...
        <p className="mb-8 max-w-[640px] font-['Outfit',sans-serif] text-[17px] font-light leading-relaxed text-[#16140f]/80">
          Standout YC companies that have achieved iconic status and are recognized
          as household names around the world.
        </p>
          ...
          {TABS.map((tab) => (
            <Link
              ...
              className={`border-b-2 px-5 py-3 font-['Outfit',sans-serif] text-[14px] font-medium transition-colors ${
                tab.href === "/topcompanies/featured"
        ...
        <div className="relative mb-6">
          ...
          <input
            type="text"
            placeholder="Search featured companies..."
            value={searchQuery}
        ...
        {filteredCompanies.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredCompanies.map((company) => (
              <FeaturedCompanyCard key={company.slug} company={company} />
            ))}
          </div>
        )}
      </div>
...
function FeaturedCompanyCard({ company }: { company: Company }) {
  return (
    <Link
