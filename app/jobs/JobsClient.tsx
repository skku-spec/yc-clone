...
const seoRoles = roleCategories.filter((r) => r.slug !== "all");
const seoLocations = locationOptions.filter((l) => l.slug !== "all");
  ...
  return (
    <li className="my-2 flex h-auto w-full flex-col flex-nowrap rounded border border-[#ccc] bg-[#faf9f5] px-5 py-3">
      ...
      <div className="flex items-start gap-3">
        ...
        {job.logo_url ? (
          <img src={job.logo_url} alt={job.company} className="hidden h-16 w-16 shrink-0 rounded-full object-cover md:block" />
        ) : (
          <div
        ...
        {job.logo_url ? (
          <img src={job.logo_url} alt={job.company} className="h-8 w-8 shrink-0 rounded-full object-cover md:hidden" />
        ) : (
          <div
...
export default function JobsClient({ jobs }: { jobs: Job[] }) {
  ...
  const roleCategoriesWithCounts = useMemo(() => {
    return roleCategories.map((r) => ({
      ...r,
      count: r.slug === "all" ? jobs.length : jobs.filter((j) => j.role_slug === r.slug).length,
    }));
  }, [jobs]);

  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      return activeRole === "all" || job.role_slug === activeRole;
    });
  }, [activeRole, jobs]);
